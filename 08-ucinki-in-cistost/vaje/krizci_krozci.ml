...
let is_full_grid grid =
    ...
    (* Preklopimo na seznam, da lahko uporabimo knjižnico - funcija preveri za vse vrstice ce so polne *)
...
let rec winner_of_list = function
  | [] -> None
  | triple :: triples ->
      match winner_of_triple triple with
        | Some p -> Some p
        | None -> winner_of_list triples
...
type state = 
(* lahko bi dali  Onturn of player * grid ampak je boljse uporabljat recorde *)
...
let show_row (a0, a1, a2) =
  ...
  ^ " |\n"
(* String.concat je enako kot string.join v pythonu *)
...
let show_state = function
  ...
  | GameOver {result; final_grid} ->
      let winner_message = 
        match result with
        | Tie -> "Nihce ni zmagal"
        | Winner p -> "Zmagal je igralec" ^ show_player p ^ "!"
      in
...
(*----------------------------------------------------------------------------*]
 ...
 read_line je kot input v pythonu
[*----------------------------------------------------------------------------*)
...
let rec loop (state : state) : unit =
  ...
  | GameOver _ -> ()
  | OnTurn {player; grid} ->
    let row_i = choose "Izberite vrstico" in
    let col_i = choose "Izberite stolpec" in
    (*preveri veljavnost poteze*)
    match get_field row_i col_i grid with 
    | Some _ -> 
      print_string "Polje je ze zasedeno\n";
      loop state
    | None ->
