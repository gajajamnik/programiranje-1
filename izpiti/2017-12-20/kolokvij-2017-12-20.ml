...
(* 1.3) Definirajte funkcijo, ki vzame seznam parov in izračuna nov seznam parov,
 ...
 let rec vsem_zamenjaj_drugo_z_42 list = match list with 
   | [] -> []
...
(* 1.4) Definirajte funkcijo, ki varno vrne glavo seznama v primeru, ko seznam ni prazen.
 ...
 let glava list = match list with
   | [] -> None
   | x::xs -> Some x
...
(* 1.5) Definirajte funkcijo, vzame funkcijo (f: 'a -> 'b), neko vrednost (x : 'a) in
 ...
 let rec uporabi_veckrat f x n = match n with
   | 0 -> x 
...
let t = Rose (1, [])
let t' = Rose (2, [Rose (1, []); Rose (1, [])])
let t'' = Rose (3, [Rose(-1, []); Rose (2, [Rose (1, []); Rose (1, [])]); Rose (0, [])])

(* 2.3) Definirajte funkcijo, ki vrne gozd rožnega drevesa. *)
let vrni_gozd rozno_drevo =
   let Rose (_, list) = rozno_drevo in
   list
...
let rec globina (Rose(_, l)) =
      let seznam_globin gozd = List.map globina l in
      let najglobji sez = List.fold_left max 0 sez in
      1 + (najglobji (seznam_globin l)) 
...
let rec globoko_drevo n = match n with
   | 0 -> failwith "ne obstaja"
   | 1 -> Rose(1, [])
   | n -> Rose(n, [globoko_drevo (n-1)])
...
(* 2.7) Definirajte funkcijo, ki sprejme funkcijo (f : 'b -> 'a -> 'b) in začetno vrednost (acc : 'b)
   ...
   t' = Rose (2, [Rose (1, []); Rose (1, [])])
   t'' = Rose (3, [Rose(-1, []); Rose (2, [Rose (1, []); Rose (1, [])]); Rose (0, [])])
   ...
   izognite. *)
let rec zlozi f acc drevo = match drevo with
   | Rose (x, []) -> f acc x
   | Rose (x, list) -> 
      let rec iz_seznama acc list = match list with
         | [] -> x + acc
         | y::ys ->  iz_seznama (zlozi f acc drevo) ys
      in
      iz_seznama acc list
